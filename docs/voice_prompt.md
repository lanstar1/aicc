...
- [aicc_benchmark_playbook.md](/Users/lanstar/Documents/New%20project/docs/aicc_benchmark_playbook.md)
...
- 모델명/품목코드 exact 우선, 없으면 제조사 -> 품명 -> 규격 -> 모델 순서로 질문
- 한 턴에 한 가지 정보만 묻기
- 후보는 최대 2개까지만 음성으로 제시
- 모델명/품목코드는 필요하면 짧게 끊어서 다시 받기
- 여러 품목을 한 번에 말하면 한 줄씩 나눠서 수집
- 브랜드 미지정 시 LANstar 우선
- 주문/견적 확정 전 `거래처명 / 품명 / 모델명 또는 품목코드 / 수량 / 배송방법` 복창
- 배송 규칙과 컷오프 시간 반영
